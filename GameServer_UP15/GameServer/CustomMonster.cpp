...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(CustomMonsterInfo.InfoMessage != -1)
	{
		...
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.InfoMessage),CustomMonsterInfo.RewardValue1);
		}
		if(CustomMonsterInfo.RewardValue2 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.InfoMessage),CustomMonsterInfo.RewardValue2);
		}
		if(CustomMonsterInfo.RewardValue3 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.InfoMessage),CustomMonsterInfo.RewardValue3);
		}
		if(CustomMonsterInfo.RewardValue4 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.InfoMessage),CustomMonsterInfo.RewardValue4);
		}*/

		if(CustomMonsterInfo.RewardValue1 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,"Bạn Nhận Được Thêm %d WcoinC",CustomMonsterInfo.RewardValue1);
		}
		if(CustomMonsterInfo.RewardValue2 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,"Bạn Nhận Được Thêm %d WcoinP",CustomMonsterInfo.RewardValue2);
		}
		if(CustomMonsterInfo.RewardValue3 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,"Bạn Nhận Được Thêm %d WcoinG",CustomMonsterInfo.RewardValue3);
		}
		if(CustomMonsterInfo.RewardValue4 > 0)
		{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,"Bạn Nhận Được Thêm %d Ruud",CustomMonsterInfo.RewardValue4);
		}
	}

		/*gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.InfoMessage),CustomMonsterInfo.RewardValue1,CustomMonsterInfo.RewardValue2, CustomMonsterInfo.RewardValue3,CustomMonsterInfo.RewardValue4);*/
...
bool CCustomMonster::GetCustomMonsterInfo(int index,int map,CUSTOM_MONSTER_INFO* lpInfo) // OK
{
	...
	{
		// thay it->Index == index thanh (it->Index == -1 || it->Index == index) de muc index nhan gia tri -1 hoac *
