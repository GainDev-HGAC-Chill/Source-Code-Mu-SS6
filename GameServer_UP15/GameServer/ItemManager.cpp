...
Next:
	...
	int AddCount = 1; // Số lượng sẽ cộng vào bank
	int TargetItemIndex = lpItem->m_Index;
	int TargetItemLevel = lpItem->m_Level;
	
	// Nếu không phải item gốc, kiểm tra xem có phải là cum10, cum20, cum30 không
	if (GetSlotItemBank == -1)
	{
		for (size_t i = 0; i < gBCustomItemBank.mListItemBank.size(); i++)
		{
			DATA_LIST_ITEMBANK& item = gBCustomItemBank.mListItemBank[i];
			if (item.Cum10Index == lpItem->m_Index && item.Cum10Level == lpItem->m_Level)
			{
				GetSlotItemBank = i;
				TargetItemIndex = item.ItemIndex;
				TargetItemLevel = item.ItemLevel;
				AddCount = 10; // 1 cum10 = 10 item gốc
				break;
			}
			else if (item.Cum20Index == lpItem->m_Index && item.Cum20Level == lpItem->m_Level)
			{
				GetSlotItemBank = i;
				TargetItemIndex = item.ItemIndex;
				TargetItemLevel = item.ItemLevel;
			...
			else if (item.Cum30Index == lpItem->m_Index && item.Cum30Level == lpItem->m_Level)
			{
				GetSlotItemBank = i;
				TargetItemIndex = item.ItemIndex;
				TargetItemLevel = item.ItemLevel;
	...
	if (GetSlotItemBank != -1)
	{
		if (gBCustomItemBank.CheckItemBankAutoPick(aIndex, TargetItemIndex, TargetItemLevel))
		{
			if (gBCustomItemBank.CongTruBank(aIndex, TargetItemIndex, TargetItemLevel, AddCount))
			{
				lpItem->Clear();
				pMsg.result = -1;
...
{
	LPOBJ lpObj = &gObj[aIndex];
	//LogAdd(LOG_RED, "[USE_ITEM] Name:%s Slot:%d Index:%d Level:%d",lpObj->Name,lpMsg->SourceSlot,lpObj->Inventory[lpMsg->SourceSlot].m_Index,lpObj->Inventory[lpMsg->SourceSlot].m_Level);
...
Next:
	...
	else if (lpItem->m_Index == GET_ITEM(14,11)) // logic chuot phai mo box
	{
		//LogAdd(LOG_RED, "[BOX_KUNDUN_1] Name:%s Slot:%d Index:%d Level:%d",lpObj->Name,lpMsg->SourceSlot,lpItem->m_Index,lpItem->m_Level);

		if (lpItem->m_Level < 8 || lpItem->m_Level > 12)
		{
			//LogAdd(LOG_RED, "[BOX_KUNDUN_2] Wrong level");
			return;
		}

		int result = gItemBagManager.DropItemByItemIndex(
			lpItem->m_Index,
			lpItem->m_Level,
			lpObj,
			lpObj->Map,
		...
		//LogAdd(LOG_RED, "[BOX_KUNDUN_3] Drop result:%d", result);
		...
		if (result != 0)
		{
			if (gItemStack.GetItemMaxStack(lpItem->m_Index) == 0)
			{
				this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
				this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
				//LogAdd(LOG_RED, "[BOX_KUNDUN_4] Delete item OK");
			}
			else
			{
				...
				if (lpItem->m_Durability < 1)
				{
					this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
					this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
					//LogAdd(LOG_RED, "[BOX_KUNDUN_4] Delete stacked item OK");
				}
				else
				{
					this->GCItemDurSend(aIndex, lpMsg->SourceSlot, (BYTE)lpItem->m_Durability, 0);
					//LogAdd(LOG_RED, "[BOX_KUNDUN_4] Reduce stack OK");
				}
			}
		}
		else
		{
			//LogAdd(LOG_RED, "[BOX_KUNDUN_5] ItemBag not found");
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"Box of Kundun chua duoc cau hinh ItemBag");
		}
	}
