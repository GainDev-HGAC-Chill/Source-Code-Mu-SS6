...
void cChienTruongCo::MonsterDie(LPOBJ Player)
{
	...
	if (Player->Map == 97 && Player->CTCTime > 0) {
		...
		if (this->PointKillMonter > 0)
		{
			...
			if (this->LoaiTien == 4)
			{
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, this->PointKillMonter, 0, 0, 0); // WCoinC
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, 0, this->PointKillMonter, 0, 0); // WCoinP
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, 0, 0, this->PointKillMonter, 0); // Goblin

				gNotice.GCNoticeSend(Player->Index, 1, 0, 0, 0, 0, 0,
            this->MsgTPoint,
            this->PointKillMonter);
